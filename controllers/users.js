...
exports.updatePassword = (req, res) => {
    if (userHelper.isActiveSessionUser(req) && userHelper.isLocalUser(req)) {
        ...
        if (allValid) {
            const { user } = req.session;
            connection.query(`UPDATE users SET password = "${password}" WHERE id = "${user.id}" LIMIT 1;`, async (err) => {
                if (err) {
...
exports.registerNew = (req, res) => {
    ...
    if (dataValidUtil.isNewUserFormValid({ name, email, password }) === true) {
        const user = {
        ...
        connection.query('INSERT INTO users SET ?', user, async (err) => {
            if (err) {
                res.status(200).send({ success: false, msg: 'can not create the user.' });
            } else {
...
exports.resend = async (req, res) => {
    ...
    const resendResult = await emailUtil.resend(email, token);
    if (resendResult === true) {
        await logUserDataAndSaveHistory(req, (await userHelper.findUserByEmail(email))[0]);
        res.redirect('/dashboard');
    } else {
        res.redirect('/');
    }
